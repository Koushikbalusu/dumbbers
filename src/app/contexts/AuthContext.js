  ...
  const fetchUserProfile = async (authToken) => {
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        console.debug('User profile fetch was aborted (timeout)');
        // Don't remove token for timeout errors, just log and continue
        return;
      } else if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
        console.debug('Network error fetching user profile:', error.message);
        // Don't remove token for network errors, just log and continue
        return;
      } else {
        console.error('Error fetching user profile:', error);
        // Only remove token for actual authentication errors
        localStorage.removeItem('authToken');
        setToken(null);
      }
    } finally {
